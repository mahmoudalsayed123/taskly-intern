    ...
    <tr className="flex w-full items-center justify-center border-t border-background-check-password">
      ...
      <td className="flex items-center w-full">
        <div className="w-34.5 py-4 px-6">
          <p
            className={`${statusBackgroundColors[task?.status as keyof typeof statusBackgroundColors]} w-fit py-1 px-2 text-label-SM font-bold text-muted-body rounded-xs`}
          >
            {task?.status?.replaceAll("_", " ")}
          </p>
        </div>
      </td>
      <td className=" flex items-center py-4.5 px-6 w-full ">
        {task?.due_date !== null ? (
          <p className="w-32.5 text-body-MD font-normal text-muted-body">
      ...
      <td className=" flex items-center w-full">
        {task?.assignee?.name !== null ? (
          <div className="w-35  flex items-center gap-3 ps-6">
          ...
          <div className="w-35 flex items-center justify-center gap-3 ps-6">
            <div className="w-7 h-7 flex items-center justify-center rounded-xl bg-surface-highest text-label-SM font-bold text-slate-dark">
